<template>
  <div class="flex sticky items-center justify-between bg-gray-800 w-full h-18">
    ...
    <div class="flex items-center h-full uppercase font-semibold text-4xl">
      <span
        @click="match.takeTimeout('home')"
        class="select-none h-full w-19 flex justify-center items-center"
        :class="[
          match.data.value.timeoutsLeftHome
            ? 'text-gray-900 focus:bg-gray-100 bg-white  '
      ...
      >
      <span
        class="w-19 flex items-center justify-center h-full bg-blue-400 text-white select-none"
      >
      ...
      <div class="relative">
        ...
        <div
          v-if="match.data.value.twoMinutesHome.length > 0"
      ...
      <div class="relative">
        <span class="mx-4 select-none text-white">{{
          match.data.value.opponent
        }}</span>
        <div
      ...
      <span
        @click="match.takeTimeout('away')"
        class="select-none h-full w-19 flex justify-center items-center"
        :class="[
          match.data.value.timeoutsLeftAway
            ? 'text-gray-900 focus:bg-gray-100 bg-white  '
            : 'bg-gray-200 text-gray-400'
        ]"
      >
