<template>
    <div v-if="shootingTarget !== null" class="flex select-none flex-col w-full text-xl">
        <div class="relative flex flex-col bg-white px-4 py-4">
            ...
            <div class="flex w-full justify-between px-6 gap-6">
                <button @click="addShotToPlayer(goalkeeperSelected ? 'gkmiss' : 'miss')" class="relative rounded flex-1 p-4 bg-red-700 active:bg-red-900 focus:shadow-inner  font-semibold text-white">
                    {{goalkeeperSelected ? 'GOAL' : 'MISS'}}
                    <shortcut-chip v-if="ctrlHeld" :combo="goalkeeperSelected ? '⌃A' : '⌃M'" class="absolute top-1 right-1" />
                </button>
                <button @click="addShotToPlayer('gksave')" v-if="goalkeeperSelected" class="relative rounded flex-1 p-4 bg-emerald-700 active:bg-emerald-900 focus:shadow-inner  font-semibold text-white">
                    STOP
                    <shortcut-chip v-if="ctrlHeld" combo="⌃H" class="absolute top-1 right-1" />
                </button>
                <button @click="addShotToPlayer('goal')" v-else class="relative rounded p-4 flex-1 shadow-md  bg-emerald-700 font-semibold text-white active:bg-emerald-900 focus:shadow-inner focus:border-0">
                    GOAL
                    <shortcut-chip v-if="ctrlHeld" combo="⌃G" class="absolute top-1 right-1" />
                </button>
            </div>
        </div>
    </div>
    <div v-else class="flex flex-col select-none w-full text-xl" >
        <div class="relative flex flex-col bg-white justify-between p-4 h-98 ">
            ...
            %</span>
            <div v-if="statsPanel.provokesOpenned.value" class="absolute flex flex-wrap gap-5 items-center bg-white text-white font-semibold rounded p-4 h-28 -ml-2">
                <button @click="setPlayerProvokeTwoMinutes('provokeCard')" class="bg-white text-emerald-900 py-4 rounded px-2 shadow-lg border border-emerald-900 flex items-center space-x-4 relative">
                    RED <span class="ml-2 h-6 w-4 bg-red-600" />
                    <shortcut-chip v-if="shiftHeld" combo="⇧R" class="absolute -top-2 -right-2" />
                </button>
                <button @click="setPlayerProvokeTwoMinutes('provokeCard')" class="bg-white text-emerald-900 py-4 rounded px-2 shadow-lg border border-emerald-900 flex items-center space-x-4 relative">
                    BLUE <span class="ml-2 h-6 w-4 bg-blue-600" />
                    <shortcut-chip v-if="shiftHeld" combo="⇧U" class="absolute -top-2 -right-2" />
                </button>
                <button @click="increasePlayerStats('provokeCard')" class="bg-white text-emerald-900 py-4 rounded px-2 shadow-lg border border-emerald-900 flex items-center space-x-4 relative">
                    YELLOW <span class="ml-2 h-6 w-4 bg-yellow-400" />
                    <shortcut-chip v-if="shiftHeld" combo="⇧Y" class="absolute -top-2 -right-2" />
                </button>
                <button @click="increasePlayerStats('provokePenalty')" class="bg-white text-emerald-900 py-4 rounded px-2 shadow-lg border border-emerald-900 relative">
                    Penalty
                    <shortcut-chip v-if="shiftHeld" combo="⇧7" class="absolute -top-2 -right-2" />
                </button>
                <button @click="setPlayerProvokeTwoMinutes('provokeTwoMin')" class="bg-white text-emerald-900 py-4 rounded px-2 shadow-lg border border-emerald-900 relative">
                    2 MIN
                    <shortcut-chip v-if="shiftHeld" combo="⇧2" class="absolute -top-2 -right-2" />
                </button>
            </div>
            <div v-if="statsPanel.extraDefenseOpened.value" class="absolute right-0 flex flex-wrap gap-5 items-center bg-white text-white font-semibold rounded-r-4xl p-4 h-28 mr-5">
                <button @click="increasePlayerStats('defensex2')" class="bg-white h-16 text-emerald-900 py-4 rounded px-2 shadow-lg border border-emerald-900 flex items-center relative">
                    <span class="text-md">EXTRA</span>
                    <span class="text-4xl -mt-1 font-bold">+</span>
                    <shortcut-chip v-if="shiftHeld" combo="⇧E" class="absolute -top-2 -right-2" />
                </button>
                <button @click="increasePlayerStats('steal')" class="bg-white text-emerald-900 py-4 rounded px-2 shadow-lg border border-emerald-900 flex items-center relative">
                    <span class="text-md">STEAL</span>
                    <steal class="h-6 w-6 text-blue-800" />
                    <shortcut-chip v-if="shiftHeld" combo="⇧S" class="absolute -top-2 -right-2" />
                </button>
                <button @click="setPlayerProvokeTwoMinutes('block')" class="bg-white text-emerald-900 py-4 rounded px-2 shadow-lg border border-emerald-900 flex items-center relative">
                    <span class="text-md">BLOCK</span>
                    <block class="h-6 w-6 text-gray-800" />
                    <shortcut-chip v-if="shiftHeld" combo="⇧B" class="absolute -top-2 -right-2" />
                </button>
            </div>
            ...
            <div v-if="!store.selection.stats.value.general" class="h-full" >
                <div class="grid px-4 mt-2" :class="goalkeeperSelected ? 'grid-rows-2 gap-40 ' : 'grid-rows-3 gap-8'">
                        <div v-if="goalkeeperSelected" class="grid grid-cols-3 gap-16">
                            ...
                            <button class="flex flex-col items-center" @click="addShotToPlayer('gkmiss_empty')" :class="negativeStatStyle">
                                <longdistance class="h-12 w-20 text-red-600 transform -scale-x-100" />
                                <span class="text-md ">EMPTY GOAL</span>
                            </button>
                        </div>
                    <div v-if="!goalkeeperSelected" class=" grid grid-cols-4 gap-16 ">
                        <button class="relative flex flex-col items-center" @click="increasePlayerStats('defense')" :class="positiveStatStyle">
                            <wrestling class="h-12 w-12 text-emerald-800" />
                            <span class="text-md ">DEFENSE</span>
                            <shortcut-chip v-if="shiftHeld" combo="⇧D" class="absolute top-1 right-1" />
                        </button>
                        ...
                        <button class="relative flex flex-col items-center" @click="addShotToPlayer('goal_empty')" :class="positiveStatStyle">
                            <longdistance class="h-12 w-20 text-emerald-800" />
                            <span class="text-md ">GOAL LD</span>
                        </button>
                        <button class="relative flex flex-col items-center" @click="toggleProvokes()" :class="positiveStatStyle">
                            <provoke class="h-12 w-12 text-emerald-800" />
                            <span class="text-md ">PROVOKE</span>
                            <shortcut-chip v-if="shiftHeld" combo="⇧P" class="absolute top-1 right-1" />
                        </button>
                    </div>
                    <div v-if="!goalkeeperSelected" class="grid grid-cols-4 gap-16 ">

                        <button class="relative flex flex-col items-center" @click="increasePlayerStats('lostball')" :class="negativeStatStyle">
                            <lostball class="h-12 w-12 text-red-600" />
                            <span class="text-md ">LOST BALL</span>
                            <shortcut-chip v-if="shiftHeld" combo="⇧L" class="absolute top-1 right-1" />
                        </button>
                        <button class="relative flex flex-col items-center" @click="increasePlayerStats('1on1lost')" :class="negativeStatStyle">
                            <span class="text-4xl h-12  font-bold">1-1</span>
                            <span class="text-md ">1-1 LOST</span>
                        </button>
                        <button class="relative flex flex-col items-center" @click="increasePlayerStats('penaltymade')" :class="negativeStatStyle">
                            <whistle class="h-12 w-12 text-red-600" />
                            <span class="text-md ">PENALTY</span>
                            <shortcut-chip v-if="shiftHeld" combo="⇧K" class="absolute top-1 right-1" />
                        </button>
                        <button class="relative flex flex-col items-center" @click="increasePlayerStats('norebound')" :class="negativeStatStyle">
                            <norebound class="h-12 w-12 text-red-600" />
                            <span class="text-md text-nowrap">NO REB</span>
                            <shortcut-chip v-if="shiftHeld" combo="⇧N" class="absolute top-1 right-1" />
                        </button>
                    </div>
                    <div class="grid grid-cols-4 gap-16">

                        <button class="relative flex flex-col items-center" @click="setPlayerTwoMinutes()" :class="suspensionStatStyle">
                            <twofingers class="h-12 w-12 text-gray-800" />
                            <span class="text-md ">2 MIN</span>
                            <shortcut-chip v-if="shiftHeld" combo="⇧2" class="absolute top-1 right-1" />
                        </button>
                        <button class="relative flex flex-col justify-center items-center" @click="increasePlayerStats('yellowcard')" :class="suspensionStatStyle">
                            <span class="h-12 w-8 bg-yellow-500" />
                            <span class="text-md ">CARD</span>
                            <shortcut-chip v-if="shiftHeld" combo="⇧Y" class="absolute top-1 right-1" />
                        </button>
                        <button class="relative flex flex-col justify-center items-center" @click="increasePlayerStats('redcard')" :class="suspensionStatStyle">
                            <span class="h-12 w-8 bg-red-600" />
                            <span class="text-md ">CARD</span>
                            <shortcut-chip v-if="shiftHeld" combo="⇧R" class="absolute top-1 right-1" />
                        </button>
                        <button class="relative flex flex-col justify-center items-center" @click="increasePlayerStats('bluecard')" :class="suspensionStatStyle">
                            <span class="h-12 w-8 bg-blue-600" />
                            <span class="text-md ">CARD</span>
                            <shortcut-chip v-if="shiftHeld" combo="⇧U" class="absolute top-1 right-1" />
                        </button>
