<template>
  <div
    class="w-full flex flex-col bg-white py-4 rounded items-center justify-center"
  >
    <span
      @click="
        store.selection.stats.value.goal = !store.selection.stats.value.goal
      "
...
const rects = [
...

  {
    ...
    stroke: "black",
    "stroke-width": 1.14,
    transform: "matrix(-1 0 0 1 117.508 118.65)"
  },
  {
    x: -0.570437,
    y: 0.570437,
    ...
    stroke: "black",
    "stroke-width": 1.14,
    transform: "matrix(-1 0 0 1 925.25 118.65)"
  }
];
  ...
  if (props.statsMode) {
    if (props.player) {
      const shots = props.player.currentShots
        ...
        .filter(s =>
          props.shootingArea ? s.from === props.shootingArea : true
        );
      ...
      const shots = store.matches.match.value?.data.value.shots
        .filter(
          s => s.to === target && s.result !== "gkmiss" && s.result !== "gksave"
        )
        .filter(s =>
          props.shootingArea ? s.from === props.shootingArea : true
        );
