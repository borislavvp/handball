...
export const usePlayerOrder = () => {
    const reorderIds = useState<number[] | null>('playerOrder-reorderIds', () => null);
    ...
    const slotFor = (team: Team | undefined | null, playerId: number): number | null => {
        const list = orderedPlayers(team);
        const idx = list.findIndex((p) => p.id === playerId);
        return idx === -1 ? null : idx;
    };

    const playerAtSlot = (team: Team | undefined | null, slot: number): Player | null => {
        const list = orderedPlayers(team);
